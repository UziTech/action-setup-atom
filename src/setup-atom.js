...
async function addToPath(channel, folder) {
  switch (process.platform) {
    ...
    case "darwin": {
      ...
      await exec("ln", [
        "-s",
        path.join(atomPath, "atom.sh"),
        path.join(atomPath, "atom"),
      ]);
      ...
      if (process.env.GITHUB_ACTIONS) {
        ...
        await writeFileAsync(
          "../env.sh",
          ["#! /bin/bash", `export "PATH=${atomPath}:${apmPath}:$PATH"`].join(
            "\n"
          ),
          { mode: "777" }
        );
    ...
    default: {
      const display = ":99";
      await exec(
        `/sbin/start-stop-daemon --start --quiet --pidfile /tmp/custom_xvfb_99.pid --make-pidfile --background --exec /usr/bin/Xvfb -- ${display} -ac -screen 0 1280x1024x16 +extension RANDR`
      );
      let atomfolder = "atom";
      if (channel !== "stable") {
        atomfolder += `-${channel}`;
