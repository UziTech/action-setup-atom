...
async function downloadAtom(channel, folder) {
  ...
  case "win32": {
    const downloadFile = await tc.downloadTool(
        `https://atom.io/download/windows_zip?channel=${channel}`);
    await tc.extractZip(downloadFile, folder);
    break;
  }
  case "darwin": {
    const downloadFile = await tc.downloadTool(
        `https://atom.io/download/mac?channel=${channel}`);
    await tc.extractZip(downloadFile, folder);
    break;
  }
  default: {
    const downloadFile = await tc.downloadTool(
        `https://atom.io/download/deb?channel=${channel}`);
...
async function addToPath(channel, folder) {
  ...
  case "darwin": {
    ...
    atomfolder += ".app";
    const atomPath =
        path.join(folder, atomfolder, "Contents", "Resources", "app");
    await exec(
        "ln",
        [ "-s", path.join(atomPath, "atom.sh"), path.join(atomPath, "atom") ]);
  ...
  default: {
    const display = ":99";
    await exec(
        `/sbin/start-stop-daemon --start --quiet --pidfile /tmp/custom_xvfb_99.pid --make-pidfile --background --exec /usr/bin/Xvfb -- ${
            display} -ac -screen 0 1280x1024x16 +extension RANDR`);
    let atomfolder = "atom";
    if (channel !== "stable") {
      atomfolder += `-${channel}`;
    }
